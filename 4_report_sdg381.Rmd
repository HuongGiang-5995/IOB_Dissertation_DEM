...
```{r}
# model result
sdg381_base <- read_rds(file.path(dir_model, "sdg381_base.rds"))
sdg381_dah <- read_rds(file.path(dir_model, "sdg381_total.aid.for.health.rds"))
sdg381_dah.vol <- read_rds(file.path(dir_model, "sdg381_total.aid.for.health.vol.rds"))
```
...
```{r}
# setup
pred <- as.data.frame(Effect(c("log_prepaid.private.health.spending"), 
                      sdg381_base, 
                      xlevels = 20)) 
pred <- pred %>%
  mutate(var = exp(pred[,1]),
...
ggplot() + 
  ...
  geom_ribbon(data = pred, aes(x = var,
              ...
              alpha = 0.3) +
  theme_classic() +
  labs(x = "Prepaid Private Health Spending per GDP (%)",
       y = "SDG 3.8.1")
plot_list <- append(plot_list, list(last_plot()))
...
```{r}
# setup
pred <- as.data.frame(Effect(c("log_sdi"), 
                      sdg381_base, 
                      xlevels = 20)) 
pred <- pred %>%
  mutate(var = exp(pred[,1]),
...
```{r}
p_patch <- ( plot_list[[1]] | plot_list[[2]] ) / ( plot_list[[3]] | plot_list[[4]] ) + plot_annotation(tag_levels = 'A') & ylab(NULL) 
...
# save plot
ggsave(filename = "./report/sdg381_baseline.png", 
       plot = p_patch,
       width = 20,
       height = 14,
       units = "cm",
...
```{r}
# setup
pred <- as.data.frame(Effect(c("log_total.aid.for.health", "log_gdp.per.capita"), 
                      sdg381_dah,
                      xlevels = 21))
...
( plot_list[[1]] )
# save plot
ggsave(filename = "./report/sdg381_dah_dah_gdp.png", 
       plot = last_plot(),
       width = 18,
       height = 10,
       units = "cm",
...
( plot_list[[2]] )
# save plot
ggsave(filename = "./report/sdg381_dah_inst.png", 
       plot = last_plot(),
       width = 18,
       height = 10,
       units = "cm",
...
( plot_list[[3]] )
# save plot
ggsave(filename = "./report/sdg381_dah_ghes.png", 
       plot = last_plot(),
       width = 18,
       height = 10,
       units = "cm",
...
```{r}
# setup
pred <- as.data.frame(Effect(c("log_total.aid.for.health.volatility", "institutional.index"), 
                      sdg381_dah.vol,
                      xlevels = 21))
...
( plot_list[[1]] )
# save plot
ggsave(filename = "./report/sdg381_dah.vol_dah_gdp.png", 
       plot = last_plot(),
       width = 18,
       height = 10,
       units = "cm",
...
( plot_list[[2]] )
# save plot
ggsave(filename = "./report/sdg381_dah.vol_dah_inst.png", 
       plot = last_plot(),
       width = 18,
       height = 10,
       units = "cm",
...
( plot_list[[3]] )
# save plot
ggsave(filename = "./report/sdg381_dah.vol_dah.vol_inst.png", 
       plot = last_plot(),
       width = 18,
       height = 10,
       units = "cm",
